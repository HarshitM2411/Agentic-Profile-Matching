...
SECTION_PATTERNS = {
    ...
    "skills": (
        r"(skills?|technical\s*skills?|technologies|competencies|core\s*competencies|"
        r"key\s*skills?|expertise|tech\s*stack|proficien\w*|tools?|frameworks?|"
        r"libraries|capabilities|areas?\s*of\s*expertise|technical\s*proficien\w*|"
        r"programming\s*languages?|software\s*skills?)"
    ),
...
def load_resume_text(file_path: str) -> str:
    ...
    try:
        ...
        if extension == ".pdf":
            ...
            return _normalize_extractable_text("\n".join(pages))
...
def _normalize_extractable_text(text: str) -> str:
    """Normalize PDF/DOCX extraction artifacts before metadata and chunking."""
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    text = re.sub(r"[\uf0b7\u2022\u25cf\u25aa\u25e6]", "\n", text)
    text = re.sub(r"[ \t]+\n", "\n", text)
    text = re.sub(r"\n{3,}", "\n\n", text)
...
def _match_section_heading(line: str) -> str | None:
    ...
    # PDF exports often use ALL-CAPS headings without punctuation
    letters = re.sub(r"[^A-Za-z]", "", cleaned)
    if letters and letters.isupper() and len(cleaned) <= 40:
        for label, pattern in SECTION_PATTERNS.items():
            if re.search(pattern, cleaned, flags=re.IGNORECASE):
                return label
    return None
...
def _extract_skills(text: str) -> str:
    ...
    normalized_text = _normalize_extractable_text(text)
    section_text = _extract_section_text(normalized_text, "skills")
    section_skills = _find_skills_in_text(section_text) if section_text else []
    full_text_skills = _find_skills_in_text(normalized_text)
...
def extract_metadata(text: str, resume_path: str) -> dict:
    ...
    return {
        "candidate_name": _extract_candidate_name(normalized_text, normalized_path),
        "skills": _extract_skills(normalized_text),
        "experience_years": _extract_experience_years(normalized_text),
...
def _normalize_text(text: str) -> str:
    return _normalize_extractable_text(text)
