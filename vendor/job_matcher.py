...
def _is_uploaded_resume(resume_path: str) -> bool:
    """Uploaded resumes are PDF/DOCX; bundled samples are plain text."""
    return Path(resume_path).suffix.lower() in {".pdf", ".docx"}


def _must_have_filter(
    chunk: dict,
    requirements: dict,
    *,
    strict_filter: bool = True,
) -> tuple[bool, float, str]:
    """Apply constraints using candidate metadata attached to the chunk."""
    ...
    penalty_only = (not strict_filter) or _is_uploaded_resume(resume_path)
...
def _search_resume_chunks(
    ...
    *,
    ...
    for chunk in chunks:
        passed_filter, filter_penalty, filter_reason = _must_have_filter(
            chunk,
            requirements,
